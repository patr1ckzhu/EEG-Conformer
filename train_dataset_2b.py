...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.shallownet = nn.Sequential(
            ...
            nn.Conv2d(40, 40, (3, 1), (1, 1)),  # 3通道,不是22通道
            nn.BatchNorm2d(40),
            nn.ELU(),
...
# ========== 训练类 ==========
class ExP2b():
    def __init__(self, nsub, save_dir='./models_2b'):
    ...
    def interaug(self, timg, label):
        ...
        aug_data = []
        aug_label = []
        for cls4aug in range(2):  # 2类
            cls_idx = np.where(label == cls4aug + 1)
            tmp_data = timg[cls_idx]
            tmp_label = label[cls_idx]

            tmp_aug_data = np.zeros((int(self.batch_size / 2), 1, 3, 1000))  # 3通道
    ...
    def get_source_data(self):
        ...
        print(f"Training data: {self.allData.shape}, labels: {self.allLabel.shape}")
    ...
    def train(self):
        ...
        label = torch.from_numpy(label - 1)  # 转为0,1
...
def main():
    parser = argparse.ArgumentParser(description='Train EEG-Conformer on Dataset 2b')
    parser.add_argument('--subjects', type=str, default='all', help='Subject IDs (e.g., "1,3,5" or "all")')
    parser.add_argument('--save_dir', type=str, default='./models_2b', help='Directory to save models')
    ...
    print("=" * 80)
    print("EEG-Conformer Training on Dataset 2b (Left Hand vs Right Hand)")
    print("=" * 80)
    print(f"Subjects: {subjects}")
    print(f"Save directory: {args.save_dir}")
    print(f"Classes: 2 (Left Hand, Right Hand)")
    ...
    for i in subjects:
        ...
        exp = ExP2b(i, save_dir=args.save_dir)
