"""
改进版 Dataset 2b 训练脚本 - GitHub Issue #40

改进内容:
1. 简化全连接层 (2440 -> 2 直接映射)
2. Pre-Norm 架构 (LayerNorm before MHA)
3. 增加数据增强频率 (3x)
4. 添加验证集 (20% of training data)

预期提升: +5-10% 准确率
Dataset 2b: 2类 (左手 vs 右手), 3通道 (C3, Cz, C4)
"""
...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.shallownet = nn.Sequential(
            ...
            nn.Conv2d(40, 40, (3, 1), (1, 1)),  # 3通道 for Dataset 2b
            nn.BatchNorm2d(40),
            nn.ELU(),
...
class TransformerEncoderBlock(nn.Sequential):
    def __init__(self, emb_size, num_heads=5, drop_p=0.5, forward_expansion=4, forward_drop_p=0.5):
        super().__init__(
            # Pre-Norm architecture (LayerNorm before MHA)
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size, n_classes):
        ...
        self.fc = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(2440, n_classes)  # Direct mapping! (2440 -> 2 for Dataset 2b)
        )
...
# ========== 改进的训练类 ==========
class ExP2b():
    def __init__(self, nsub, save_dir='./models_2b_improved', validate_ratio=0.2):
        """
        改进的 Dataset 2b 训练类
        ...
        """
        super(ExP2b, self).__init__()
        self.batch_size = 100
        self.n_epochs = 2000
        self.c_dim = 2  # 2 classes
        self.lr = 0.0002
        self.b1 = 0.5
        self.b2 = 0.999
        self.nSub = nsub
        ...
        self.model = Conformer2b().cuda()
        self.model = nn.DataParallel(self.model, device_ids=[i for i in range(len(gpus))])
        self.model = self.model.cuda()

    def interaug(self, timg, label):
        """Segmentation and Reconstruction (S&R) data augmentation for 2 classes"""
        aug_data = []
        aug_label = []
        for cls4aug in range(2):  # 2 classes
            cls_idx = np.where(label == cls4aug + 1)
            tmp_data = timg[cls_idx]
            tmp_label = label[cls_idx]

            tmp_aug_data = np.zeros((int(self.batch_size / 2), 1, 3, 1000))  # 3 channels
    ...
    def get_source_data(self):
        ...
        # Load training data: session 1,2,3
        train_data = []
        train_label = []
        ...
        print(f"Training data: {self.trainData.shape}, labels: {self.trainLabel.shape}")
        print(f"Validation data: {self.valData.shape}, labels: {self.valLabel.shape}")
        print(f"Test data: {self.testData.shape}, labels: {self.testLabel.shape}")
    ...
    def train(self):
        ...
        stats = {
            ...
            'dataset': '2b',
            'improvements': {
                '1_simplified_fc': 'Single layer FC (2440->2)',
                '2_pre_norm': 'LayerNorm before MHA',
        ...
        print(f'\n{"="*60}')
        print(f'Subject {self.nSub} Training Complete (IMPROVED MODEL - Dataset 2b):')
...
def main():
    parser = argparse.ArgumentParser(description='Train improved EEG-Conformer on Dataset 2b (Issue #40)')
    parser.add_argument('--subjects', type=str, default='all', help='Subject IDs (e.g., "1,3,5" or "all")')
    parser.add_argument('--save_dir', type=str, default='./models_2b_improved', help='Directory to save models')
    ...
    print("=" * 80)
    print("EEG-Conformer IMPROVED MODEL Training - Dataset 2b (GitHub Issue #40)")
    print("=" * 80)
    print("Improvements:")
    print("  1. Simplified FC layer (2440 -> 2 direct mapping)")
    ...
    print("=" * 80)
    print(f"Dataset: 2b (Left Hand vs Right Hand)")
    print(f"Channels: 3 (C3, Cz, C4)")
    ...
    result_write.write("IMPROVED MODEL - Dataset 2b - GitHub Issue #40\n")
    result_write.write("="*80 + "\n")
    result_write.write("Improvements:\n")
    result_write.write("1. Simplified FC layer (2440 -> 2 direct)\n")
    result_write.write("2. Pre-Norm architecture (LayerNorm before MHA)\n")
    ...
    for i in subjects:
        ...
        exp = ExP2b(i, save_dir=args.save_dir, validate_ratio=args.validate_ratio)
    ...
    try:
        baseline_summary_path = './models_2b/training_summary.txt'
